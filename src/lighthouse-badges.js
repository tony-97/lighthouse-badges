...
const {
  ReportGenerator,
} = require("lighthouse/report/generator/report-generator");
...
const metricsToSvg = async (
  ...
  if (badgeStyle == "pagespeed") {
    const pagespeed_svg = proceed(
      `${lighthouseMetrics["lighthouse performance"]}`,
      `${lighthouseMetrics["lighthouse accessibility"]}`,
      `${lighthouseMetrics["lighthouse best-practices"]}`,
      `${lighthouseMetrics["lighthouse seo"]}`,
      `${lighthouseMetrics["lighthouse pwa"]}`,
    );
    const filepath = path.join(outputPath, `pagespeed.svg`);
    fs.writeFile(filepath, pagespeed_svg, (error) =>
      statusMessage(
    ...
    R.keys(lighthouseMetrics).map((lighthouseMetricKey) => {
      const filepath = path.join(
        outputPath,
        `${lighthouseMetricKey.replace(/ /g, "_")}.svg`,
      );
      const badgeColor = percentageToColor(
        lighthouseMetrics[lighthouseMetricKey],
      );
      const badgeLabel =
        precedingLabel +
        lighthouseMetricKey.charAt(11).toUpperCase() +
        lighthouseMetricKey.slice(12);
...
const processRawLighthouseResult = async (data, url, shouldSaveReport) => {
  const htmlReport = shouldSaveReport
    ? ReportGenerator.generateReportHtml(data)
    : false;
...
const calculateLighthouseMetrics = async (
  url,
  shouldSaveReport,
  additionalParams = "",
) => {
...
const processParameters = async (args, func) => {
  ...
  const results = await Promise.all(
    args.urls.map((url) => func(url, args.save_report, additionalParams)),
  );
  ...
  const metricsResults =
    args.single_badge && args.badge_style != "pagespeed"
  ...
  await generateArtifacts({
    ...
    svg: {
      ...
      precedingLabel: args.preceding_label,
    },
    outputPath,
  });
};
